## /* @function
 ...
 #  @examples
 #  # ignore errors
 #  _::short_ans "Enter your Git user name:"
 #  if [[ -n "$_ans" ]]; then
 ...
 #  if _::short_ans "Enter your Git user name:" && [[ -n "$_ans" ]]; then
 #      git config --global user.name "$_ans"
 #      ...
 #  fi
 #  examples@
 ...
 #  returns@
 #
 #  @file functions/utility/_::short_ans.sh
 ## */

function _::short_ans() {
  declare -i turn=0
  local prompt
