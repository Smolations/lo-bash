## /* @function
 #  @usage _.inArgs <arg_name> "$@"
 #  @usage _.inArgs <arg_name> "${_args_clipped[@]}"
 #
 #  @output false
 #
 #  @exports
 ...
 #  `_.inArgs`. Simply pass name of the option (without any leading hyphens) along
 ...
 #
 #  If further parsing is required, `_.inArgs` conveniently exports the original
 ...
 #  can then pass the exported variable into your next `_.inArgs` call, ALWAYS
 ...
 #  # they are not emulating consecutive calls to `_.inArgs` ina  single script.
 #  # assume arguments passed to a script were:
 #  #   -a --quiet --dry-run=first
 #  _.inArgs quiet "$@"          # returns 0 (success) matches: --quiet
 #                                #          $_arg_val    =
 #                                #        $_arg_index    = 1
 #                                #   ${_args_clipped[@]} = -a --dry-run=first
 #
 #  _.inArgs a "$@"              # returns 0 (success) matches: -a
 #                                #          $_arg_val    =
 #                                #        $_arg_index    = 0
 #                                #   ${_args_clipped[@]} = --quiet --dry-run=first
 #
 #  _.inArgs -a "$@"             # returns 2 (failure) should not include hyphen
 #                                #          $_arg_val    =
 #                                #        $_arg_index    =
 #                                #   ${_args_clipped[@]} = -a --quiet --dry-run=first
 #
 #  _.inArgs dry-run "$@"        # returns 0 (success) matches: --dry-run
 #                                #          $_arg_val    = first
 #                                #        $_arg_index    = 2
 ...
 #  if _.inArgs a "$@"; then
 #      if _.inArgs quiet "${_args_clipped[@]}"; then
 #          git add -A . &> /dev/null
 #      else
 #          git add -A .
 #      fi
 #  fi
 #
 #  if _.inArgs dry-run "${_args_clipped[@]}"; then
 #      runType="$_arg_val"
