## /* @function
 #  @usage _inArgs <arg_name> "$@"
 #  @usage _inArgs <arg_name> "${_args_clipped[@]}"
 #
 #  @output false
 #
 #  @exports
 ...
 #  `_inArgs`. Simply pass name of the option (without any leading hyphens) along
 ...
 #
 #  If further parsing is required, `_inArgs` conveniently exports the original
 ...
 #  can then pass the exported variable into your next `_inArgs` call, ALWAYS
 ...
 #  # they are not emulating consecutive calls to `_inArgs` ina  single script.
 #  # assume arguments passed to a script were:
 #  #   -a --quiet --dry-run=first
 #  _inArgs quiet "$@"          # returns 0 (success) matches: --quiet
 #                                #          $_arg_val    =
 #                                #        $_arg_index    = 1
 #                                #   ${_args_clipped[@]} = -a --dry-run=first
 #
 #  _inArgs a "$@"              # returns 0 (success) matches: -a
 #                                #          $_arg_val    =
 #                                #        $_arg_index    = 0
 #                                #   ${_args_clipped[@]} = --quiet --dry-run=first
 #
 #  _inArgs -a "$@"             # returns 2 (failure) should not include hyphen
 #                                #          $_arg_val    =
 #                                #        $_arg_index    =
 #                                #   ${_args_clipped[@]} = -a --quiet --dry-run=first
 #
 #  _inArgs dry-run "$@"        # returns 0 (success) matches: --dry-run
 #                                #          $_arg_val    = first
 #                                #        $_arg_index    = 2
 ...
 #  if _inArgs a "$@"; then
 #      if _inArgs quiet "${_args_clipped[@]}"; then
 #          git add -A . &> /dev/null
 #      else
 #          git add -A .
 #      fi
 #  fi
 #
 #  if _inArgs dry-run "${_args_clipped[@]}"; then
 #      runType="$_arg_val"
